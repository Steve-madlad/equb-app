...
export async function createEqub(config: EqubConfig, createdBy: string): Promise<Equb> {
...
export async function rejectMembership(
  ...
  const rejectionReason = reason?.trim() || 'Your Equb membership request was rejected.';
  const now = new Date().toISOString();
  const updates = {
...
export async function updateEqub(
  ...
  const nextConfig: EqubConfig = {
    ...
    contributionAmountMinor: updates.contributionAmountMinor ?? current.contributionAmountMinor,
    currency: current.currency,
    frequency: updates.frequency ?? current.frequency,
    customIntervalDays: updates.customIntervalDays ?? current.customIntervalDays,
    numberOfCycles: updates.numberOfCycles ?? current.numberOfCycles,
    memberLimit: updates.memberLimit ?? current.memberLimit,
    minimumMemberCount: updates.minimumMemberCount ?? current.minimumMemberCount,
...
export async function deleteEqub(equbId: string, actorId: string): Promise<void> {
  const db = getAdminDb();
  const current = await getEqub(equbId);
  if (!current) throw new Error('Equb not found');
...
export async function openEqubForMembers(equbId: string, actorId: string): Promise<Equb> {
...
export async function lockEqub(
  ...
  if (!canStartEqub(memberships.length, equb.minimumMemberCount, equb.memberLimit)) {
    throw new Error(
  ...
  const rejectedCount = await rejectPendingMembershipsOnActivation(equbId, equb.name, actorId);
  if (rejectedCount > 0) {
    await createAuditLog({
      action: 'EQUB_UPDATED',
      actorId,
...
export async function requestMembership(equbId: string, userId: string): Promise<Membership> {
  const equb = await getEqub(equbId);
  if (!equb) throw new Error('Equb not found');
  if (!canJoinEqub(equb.status)) throw new Error('Equb is not accepting members');
  ...
  await db.collection(COLLECTIONS.memberships).doc(membership.id).set(membership);
  await createAuditLog({
...
export async function approveMembership(
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const membership = doc.data() as Membership;
    if (membership.status !== 'PENDING') throw new Error('Membership not pending');
...
export async function leaveMembership(membershipId: string, userId: string): Promise<void> {
  const db = getAdminDb();
  const ref = db.collection(COLLECTIONS.memberships).doc(membershipId);
  const doc = await ref.get();
  if (!doc.exists) throw new Error('Membership not found');
...
export async function getMembershipsForEqub(equbId: string): Promise<Membership[]> {
  const db = getAdminDb();
  const snapshot = await db.collection(COLLECTIONS.memberships).where('equbId', '==', equbId).get();
...
export async function getPendingMembershipRequestsForAdmin(
  ...
  return requests.sort((a, b) => b.membership.joinedAt.localeCompare(a.membership.joinedAt));
}
...
export async function getCurrentCycle(equbId: string): Promise<Cycle | null> {
  ...
  const snapshot = await db
    .collection(COLLECTIONS.cycles)
    .where('equbId', '==', equbId)
    .where('status', 'in', ['ACTIVE', 'DRAW_PENDING', 'WAITING_FOR_ELIGIBILITY'])
    .limit(1)
    .get();
