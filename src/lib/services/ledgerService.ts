...
export async function getLedgerEntriesForUser(userId: string): Promise<LedgerEntry[]> {
  const db = getAdminDb();
  const snapshot = await db.collection(COLLECTIONS.ledger).where('userId', '==', userId).get();
...
export async function getSettledContributionTotalForCycle(cycleId: string): Promise<MoneyMinor> {
  const db = getAdminDb();
  const snapshot = await db.collection(COLLECTIONS.ledger).where('cycleId', '==', cycleId).get();

  return snapshot.docs.reduce<MoneyMinor>((total, doc) => {
    const entry = doc.data() as LedgerEntry;
    return entry.type === 'CONTRIBUTION_RECEIVED' ? total + entry.amountMinor : total;
  }, 0 as MoneyMinor);
}
