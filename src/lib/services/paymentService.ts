...
export async function initiatePayment(
  ...
  const db = getAdminDb();
  const obligationRef = db.collection(COLLECTIONS.obligations).doc(obligationId);
  ...
  if (obligation.status === 'PENDING' && isObligationOverdue(obligation, currentDateIso)) {
...
export async function verifyAndRecordPayment(
  ...
  const obligationRef = db.collection(COLLECTIONS.obligations).doc(payment.obligationId);
  const obligationDoc = await obligationRef.get();
  const obligation = obligationDoc.data() as ContributionObligation;
  if (obligation.status === 'PENDING' && isObligationOverdue(obligation, currentDateIso)) {
    await markObligationOverdue(obligationRef, obligation);
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (
      ...
      const freshObligation = freshObligationDoc.data() as ContributionObligation;
    ...
    await createAuditLog({
      ...
      affectedUserId: freshPayment.userId,
      entityId: freshPayment.id,
      ...(verification.failureReason ? { metadata: { reason: verification.failureReason } } : {}),
    });
...
export async function markOverdueObligationsForEqub(
  ...
  const db = getAdminDb();
  const snapshot = await db.collection(COLLECTIONS.obligations).where('equbId', '==', equbId).get();
...
export async function getObligationsForCycle(cycleId: string): Promise<ContributionObligation[]> {
  const db = getAdminDb();
  const snapshot = await db
    .collection(COLLECTIONS.obligations)
