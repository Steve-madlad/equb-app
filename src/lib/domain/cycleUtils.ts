...
export function calculatePoolAmount(contributionAmountMinor: number, memberCount: number): number {
...
export function validateEqubConfig(config: Partial<EqubConfig>): string[] {
  ...
  if (!config.memberLimit || config.memberLimit < 2) errors.push('Member limit must be at least 2');
