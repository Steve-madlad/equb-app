...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 text-slate-100 selection:bg-emerald-500 selection:text-white">
      <HomeNavbar />

      <main className="relative overflow-hidden">
        {/* Ambient Glow Spheres */}
        <div className="absolute top-20 left-1/2 -translate-x-1/2 w-[800px] h-[450px] bg-gradient-to-tr from-emerald-600/20 via-teal-500/15 to-emerald-400/10 rounded-full blur-[140px] pointer-events-none -z-10" />
        <div className="absolute top-[600px] left-10 w-96 h-96 bg-emerald-700/10 rounded-full blur-[120px] pointer-events-none -z-10" />

        {/* ─── Hero Section ──────────────────────────────────────────────── */}
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-16 pb-24 text-center">
          {/* Top Pill Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 backdrop-blur-md text-emerald-300 text-xs font-semibold uppercase tracking-wider mb-8 shadow-sm">
          ...
          <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            <Link href="/search" className="w-full sm:w-auto">
              <Button
                size="lg"
                variant="secondary"
                className="w-full sm:w-auto px-7 py-3.5 rounded-xl border border-white/15 bg-white/[0.05] hover:bg-white/[0.1] text-slate-200 backdrop-blur-md font-medium text-base transition-all"
              >
          ...
          <div className="mt-16 mx-auto max-w-4xl rounded-3xl border border-white/15 bg-gradient-to-b from-white/[0.08] to-white/[0.02] backdrop-blur-2xl p-6 sm:p-8 shadow-2xl text-left">
              ...
              <div className="flex items-center gap-3.5">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-lg font-bold text-white">Addis Tech Innovators Equb</h3>
                    <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[11px] font-semibold">
                      ACTIVE
                    </span>
                  </div>
                  <p className="text-xs text-slate-400 mt-0.5">Cycle 4 of 12 • Monthly Frequency</p>
                </div>
              </div>

              <div className="sm:text-right">
                <div className="text-xs font-medium text-slate-400">Current Pool Amount</div>
                <div className="text-2xl font-black text-emerald-400">50,000.00 ETB</div>
              </div>
            </div>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-6">
              <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
                <div className="text-[11px] text-slate-400">Contribution / Member</div>
                <div className="text-lg font-bold text-white mt-1">5,000 ETB</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
                <div className="text-[11px] text-slate-400">Total Members</div>
                <div className="text-lg font-bold text-white mt-1">10 Verified</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
                <div className="text-[11px] text-slate-400">Next Scheduled Draw</div>
                <div className="text-lg font-bold text-emerald-400 mt-1">In 3 Days</div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-slate-900/60 p-4">
                <div className="text-[11px] text-slate-400">Draw Security</div>
                <div className="text-lg font-bold text-teal-300 mt-1 flex items-center gap-1">
            ...
            <div className="mt-6 flex items-center justify-between text-xs text-slate-400 bg-white/[0.02] border border-white/5 rounded-xl px-4 py-2.5">
              <span className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                <span>Next recipient draw will be automatically deposited to CBE / Telebirr</span>
              </span>
              <span className="text-emerald-400 font-mono text-[11px] hidden sm:inline">Provably Fair Verification ✅</span>
            </div>
          </div>
        </section>

        {/* ─── Supported Banks Strip ──────────────────────────────────────── */}
        <section className="border-y border-white/10 bg-slate-900/40 backdrop-blur-md py-8">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-6">
              {institutions.map((bank) => (
                <div
                  key={bank.name}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl border border-white/10 bg-white/[0.04] text-xs font-medium text-slate-300 hover:border-emerald-500/40 hover:text-emerald-300 transition-colors"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {features.map((item) => {
              ...
              return (
                <div
                  key={item.title}
                  className="group relative rounded-3xl border border-white/10 bg-white/[0.03] hover:bg-white/[0.06] hover:border-emerald-500/30 backdrop-blur-xl p-7 transition-all duration-300 shadow-xl flex flex-col justify-between"
                >
                  <div>
                    <div className="flex items-center justify-between mb-5">
                      <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-center text-emerald-400 group-hover:scale-110 group-hover:bg-emerald-500/20 transition-all">
                        <Icon className="w-6 h-6" />
                      </div>
                      <span className="text-[10px] font-bold tracking-wider px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 uppercase">
                        {item.badge}
                      </span>
                    </div>
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {steps.map((s, idx) => (
                <div
                  key={s.step}
                  className="relative rounded-3xl border border-white/10 bg-white/[0.02] p-6 backdrop-blur-xl hover:border-emerald-500/40 transition-all"
                >
                  <div className="text-3xl font-black text-emerald-500/40 mb-3 font-mono">
                    {s.step}
                  </div>
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {/* Traditional Card */}
            <div className="rounded-3xl border border-red-500/20 bg-red-500/[0.02] backdrop-blur-xl p-8">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-xl bg-red-500/10 text-red-400 flex items-center justify-center font-bold">
                  ✕
                </div>
                <h3 className="text-xl font-bold text-white">Traditional Manual Equb</h3>
              </div>
            ...
            <div className="rounded-3xl border border-emerald-500/40 bg-emerald-500/[0.04] backdrop-blur-xl p-8 shadow-xl shadow-emerald-500/5">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-10 rounded-xl bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold">
                  ✓
                </div>
                <h3 className="text-xl font-bold text-white">Modern Digital Equb</h3>
              </div>
            ...
            <div className="space-y-4">
              {faqs.map((faq, index) => {
                ...
                return (
                  <div
                    key={faq.q}
                    className="rounded-2xl border border-white/10 bg-white/[0.02] backdrop-blur-xl overflow-hidden transition-all"
                  >
                    <button
        ...
        <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
          <div className="relative rounded-3xl border border-emerald-500/30 bg-gradient-to-r from-emerald-900/60 via-slate-900/80 to-teal-900/60 backdrop-blur-2xl p-8 sm:p-14 text-center overflow-hidden shadow-2xl">
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/register" className="w-full sm:w-auto">
                <Button
                  size="lg"
                  className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-gradient-to-r from-emerald-400 to-teal-500 hover:from-emerald-300 hover:to-teal-400 text-slate-950 font-bold shadow-lg shadow-emerald-500/30 transition-all text-base"
                >
                  Create Free Account
                </Button>
              </Link>
              <Link href="/how-it-works" className="w-full sm:w-auto">
                <Button
                  size="lg"
                  variant="secondary"
                  className="w-full sm:w-auto px-7 py-3.5 rounded-xl border border-white/20 bg-white/10 hover:bg-white/15 text-white font-medium text-base transition-all"
                >
                  Learn More
                </Button>
              </Link>
      ...
      {/* ─── Footer ─────────────────────────────────────────────────────── */}
      <footer className="border-t border-white/10 bg-slate-950/80 py-12 text-center text-xs text-slate-400">
