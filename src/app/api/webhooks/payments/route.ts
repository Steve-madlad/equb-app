...
import { completePayout, getPayoutByTransferReference } from '@/lib/services/payoutService';
...
export async function GET(request: NextRequest) {
  const providerTransactionId =
    request.nextUrl.searchParams.get('tx_ref') ?? request.nextUrl.searchParams.get('trx_ref');
    ...
    if (isTransferEvent) {
      ...
      if (!transferRef) {
        return NextResponse.json({
          received: true,
          processed: false,
          reason: 'Missing transfer reference',
        });
      }
      ...
      if (
        ...
        await getAdminDb().collection(COLLECTIONS.payouts).doc(payout.id).update({
          status: 'FAILED',
          failureReason,
