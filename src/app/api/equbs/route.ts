...
export async function GET(request: NextRequest) {
  try {
    ...
    const equbs = await listEqubs(status as import('@/lib/domain/types').EqubStatus | undefined);
    const equbSummaries = await Promise.all(
      equbs.map(async (equb) => {
        ...
        return {
          ...
          activeMemberCount,
          pendingMemberCount,
          createdByName: creator?.displayName ?? creator?.email ?? equb.createdBy,
        };
      }),
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to create Equb';
