...
export default function EqubDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const unsub = onIdTokenChanged(getFirebaseAuth(), async (user) => {
      ...
      if (profileRes.ok) {
        ...
        setUserName(
          profile.displayName ?? profile.email ?? user.displayName ?? user.email ?? 'Account',
        );
      }
      loadData(equbId, t);
    ...
    try {
      ...
      if (res.ok) {
        ...
        setActivePayment({
          ...
          amountMinor: body.payment.amountMinor ?? matchingObligation?.totalDueMinor ?? 0,
          dueDate:
  ...
  async function handlePaymentOutcome(outcome: 'SUCCESS' | 'FAILED') {
    ...
    if (res.ok) {
      toast.success(outcome === 'SUCCESS' ? 'Payment confirmed.' : 'Payment marked failed.');
      ...
      return (
        <div className="min-h-screen bg-slate-100/70 dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950">
          ...
          <main className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-xl items-center px-4 py-12">
            <div className="w-full rounded-3xl border border-slate-200/90 bg-white p-8 text-center shadow-md backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.03]">
  ...
  const allPendingSelected =
    ...
    pendingMemberRows.every(({ membership }) => selectedMembershipIds.includes(membership.id));
  const selectedPendingCount = selectedMembershipIds.length;
  const currentCycleObligation = currentCycle
    ? userObligations.find((obligation) => obligation.cycleId === currentCycle.id)
    : undefined;
  const pendingObligations =
    currentCycleObligation && currentCycleObligation.status !== 'PAID'
      ? [currentCycleObligation]
      : [];
  const overdueObligations = userObligations.filter((o) => o.status === 'OVERDUE');
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 transition-colors duration-300 dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-emerald-950">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* Hero Header */}
        <div className="mb-8 rounded-3xl border border-slate-200/90 bg-white p-6 shadow-md shadow-slate-200/60 backdrop-blur-xl sm:p-8 dark:border-white/10 dark:bg-white/[0.04] dark:shadow-xl dark:shadow-black/25">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {isAdmin && (
                ...
                >
                  {!actionLoading && <Trash2 className="mr-1 h-3.5 w-3.5" />}
                  Delete
                </Button>
              )}
            </div>
          ...
          {/* Metric Tiles Grid */}
          <div className="mt-8 grid grid-cols-2 gap-4 border-t border-slate-200 pt-6 lg:grid-cols-4 dark:border-white/10">
            <div className="rounded-2xl border border-slate-200/70 bg-slate-50 p-4 dark:border-white/5 dark:bg-slate-900/40">
              <div className="mb-1 flex items-center gap-2 text-[10px] font-bold tracking-wider text-slate-500 uppercase dark:text-slate-400">
            ...
            <div className="rounded-2xl border border-slate-200/70 bg-slate-50 p-4 dark:border-white/5 dark:bg-slate-900/40">
              ...
              <p className="mt-0.5 text-[11px] text-slate-500 dark:text-slate-500">
                {equb.numberOfCycles} total cycles
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-8 grid gap-6 md:grid-cols-3">
          ...
          {!isAdmin && (
            <div className="rounded-3xl border border-slate-200/90 bg-white p-6 shadow-md shadow-slate-200/60 backdrop-blur-xl md:col-span-2 dark:border-white/10 dark:bg-white/[0.03] dark:shadow-xl dark:shadow-black/25">
              ...
              {activeUserMembership ? (
                <div className="space-y-4">
                  ...
                  {eligibility?.reason && (
                    <div className="flex items-center gap-2 rounded-2xl border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-500/20 dark:bg-amber-500/10 dark:text-amber-300">
                  ...
                  {overdueObligations.length > 0 && (
                    <div className="flex items-center gap-2 rounded-2xl border border-rose-300 bg-rose-50 p-3 text-xs text-rose-800 dark:border-rose-500/20 dark:bg-rose-500/10 dark:text-rose-300">
                      <AlertCircle className="h-4 w-4 shrink-0 text-rose-500 dark:text-rose-400" />
                      <span>
                        {overdueObligations.length} overdue obligation(s) pending clearance.
                      </span>
                    </div>
                  )}
                  ...
                  {pendingObligations.slice(0, 1).map((o) => (
                    <div
                      key={o.id}
                      className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-emerald-300 bg-emerald-50/80 p-4 dark:border-emerald-500/30 dark:bg-emerald-500/10"
                    >
                      <div>
                  ...
                  {activeUserMembership &&
                    ...
                    !['ACTIVE', 'COMPLETED', 'CANCELLED'].includes(equb.status) && (
                      <Button
                        ...
                        loading={actionLoading}
                        className="rounded-xl border border-slate-200 text-xs transition-colors hover:bg-rose-50 hover:text-rose-600 dark:border-white/10 dark:hover:bg-rose-500/10 dark:hover:text-rose-400"
                      >
                ...
                <div className="space-y-4">
                  {userMembership && INACTIVE_STATUSES.includes(userMembership.status) && (
                    <div className="flex items-center gap-2 rounded-2xl border border-slate-200/70 bg-slate-50 p-3 text-xs text-slate-600 dark:border-white/5 dark:bg-slate-800/50 dark:text-slate-400">
          ...
          {isAdmin && (
            <div className="rounded-3xl border border-slate-200/90 bg-white p-6 shadow-md shadow-slate-200/60 backdrop-blur-xl md:col-span-2 dark:border-white/10 dark:bg-white/[0.03] dark:shadow-xl dark:shadow-black/25">
              ...
              <div className="space-y-4">
                {equb.status === 'DRAFT' && (
                  <div className="flex items-center justify-between gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-500/20 dark:bg-emerald-500/10">
                ...
                {equb.status === 'OPEN_FOR_MEMBERS' && (
                  <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-slate-900/50">
                    ...
                    {approvedMemberCount >= minimumApprovedMembersToStart ? (
                      ...
                      <p className="text-xs text-amber-700 dark:text-amber-400">
                        Equb requires at least {minimumApprovedMembersToStart} approved members
                        before it can start.
                      </p>
                    )}
                  </div>
                ...
                {!['DRAFT', 'OPEN_FOR_MEMBERS'].includes(equb.status) && (
                  <div className="flex flex-col items-center justify-center gap-2 py-6 text-center">
                    <div className="mb-1 flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-800/60">
                    ...
                    <p className="text-xs text-slate-400 dark:text-slate-500">
                      ...
                      <span className="font-medium capitalize">
                        {equb.status.toLowerCase().replace(/_/g, ' ')}
                      </span>{' '}
                      — lifecycle controls are locked.
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="rounded-3xl border border-slate-200/90 bg-white p-6 shadow-md shadow-slate-200/60 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.03] dark:shadow-xl dark:shadow-black/25">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Cycle Timeline</h2>
            </div>
        ...
        {isAdmin && (
          <div className="mb-8 overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-md shadow-slate-200/60 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.03] dark:shadow-xl dark:shadow-black/25">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200/90 px-6 py-4 dark:border-white/10">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  ...
                  {pendingMemberRows.length > 0
                    ? ` • ${pendingMemberRows.length} pending approval`
                    : ''}
                </p>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  className="rounded-xl border border-slate-200 text-xs font-semibold dark:border-white/10"
                >
                  {!actionLoading && (
                  ...
                  <TableRow className="border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/[0.02]">
                    <TableHead className="w-12 px-6 py-3">
                      <input
                        ...
                        className="h-4 w-4 rounded border-slate-300 bg-white text-emerald-600 focus:ring-emerald-500 dark:border-white/20 dark:bg-slate-900"
                      />
                    </TableHead>
                    <TableHead className="px-6 py-3 text-xs font-bold text-slate-600 uppercase dark:text-slate-400">
                      Member
                    </TableHead>
                    <TableHead className="px-6 py-3 text-xs font-bold text-slate-600 uppercase dark:text-slate-400">
                      Rating
                    ...
                    <TableHead className="px-6 py-3 text-right text-xs font-bold text-slate-600 uppercase dark:text-slate-400">
                      Action
                    </TableHead>
                  </TableRow>
                </TableHeader>
                  ...
                  {memberRows.length === 0 ? (
                    ...
                    memberRows.map(({ membership, user, contributionStatus }) => {
                      ...
                      return (
                        <TableRow
                          key={membership.id}
                          className="border-slate-200/70 transition-colors hover:bg-slate-50/80 dark:border-white/5 dark:hover:bg-white/[0.03]"
                        >
                          ...
                          <TableCell className="px-6 py-3.5">
                            <span className="inline-flex items-center rounded-full bg-teal-500/10 px-2 py-0.5 font-mono text-xs font-bold text-teal-700 dark:text-teal-400">
                              {user.rating}/100
                            </span>
                          </TableCell>
                            ...
                            {isPending ? (
                              <div className="flex items-center justify-end gap-2">
                                <Button
                                  ...
                                  className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-xs font-semibold text-emerald-700 hover:bg-emerald-500/20 dark:text-emerald-400"
                                >
                                  Approve
                                </Button>
                                <Button
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-md shadow-slate-200/60 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.03] dark:shadow-xl dark:shadow-black/25">
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow className="border-slate-200 bg-slate-50 dark:border-white/10 dark:bg-white/[0.02]">
                  <TableHead className="px-6 py-3.5 text-xs font-bold text-slate-600 uppercase dark:text-slate-400">
                    Cycle #
                  </TableHead>
                  <TableHead className="px-6 py-3.5 text-xs font-bold text-slate-600 uppercase dark:text-slate-400">
                    Due Date
                ...
                {cycles.length === 0 ? (
                  ...
                  cycles.map((cycle) => (
                    <TableRow
                      ...
                      className="border-slate-200/70 transition-colors hover:bg-slate-50/80 dark:border-white/5 dark:hover:bg-white/[0.03]"
                    >
                      <TableCell className="px-6 py-4 text-sm font-bold text-slate-900 dark:text-white">
                        Cycle {cycle.cycleNumber}
                      </TableCell>
                      <TableCell className="px-6 py-4 font-mono text-xs text-slate-500 dark:text-slate-400">
                        {formatDate(cycle.dueDate)}
                      </TableCell>
                      <TableCell className="px-6 py-4 text-sm font-bold text-emerald-700 dark:text-emerald-400">
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open) => {
          if (!actionLoading) setDeleteConfirmOpen(open);
        }}
        variant="danger"
